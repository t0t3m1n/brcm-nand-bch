...
void print_usage(void)
{
    ...
    printf("./encode -i infile -o outfile -b 131072 -p 2048 -s 64 -e 4 -m 14\n");
    return;
}
...
int main(int argc, char **argv)
{
    ...
    page_buffer = malloc((sector_sz + oob_sz) * SECTORS_PER_PAGE);
    if (!page_buffer)
    {
    ...
    buffer = malloc(sector_sz + oob_ecc_ofs);
    if (!buffer)
    {
        printf("Failed to allocate the  buffer\n");
        goto exit;
    ...
    while (!feof(in_file))
    {
        ...
        if (numBytesRead == 0)
        {
            fprintf(stdout, "\n\nPoly %d different OOB sectors: %d\n", BROADCOM_POLY, differing);
            printf("\r\nFread failed or file size is zero\r\n");
            goto exit;
        }

        if (numBytesRead != (sector_sz + oob_sz) * SECTORS_PER_PAGE)
            break; /* skip if bytes read from fread api is less than page size */
        ...
        for (i = 0; i != SECTORS_PER_PAGE; ++i)
        {
            ...
            {
                memset(buffer, 0, sector_sz + oob_ecc_ofs); //512 + 9 oob bytes
